...
| `00057`        | Story 3.6 format correction — re-emits `batch_register_devices` with the trailing `M` removed from the generated serial-number format (`{article_number}-{MMYY}-{NNNNN}` instead of `{article_number}M-{MMYY}-{NNNNN}`). Tracks David's MTG-009 decision (2026-04-28): "M/K-Suffix verschwindet aus Artikelnummern UND Seriennummern. Miete vs. Kauf wird über separaten Systemwert am Einzelartikel gesteuert." Counter MAX regex tolerates BOTH formats (`(?:[MK])?` non-capturing optional letter) so per-article numbering continues across the change without restarting at 1. Forward-only: existing legacy `M`-format rows are NOT rewritten (printed labels would be invalidated). All 00054 patches (P1 regex-escape, P2 99999 overflow guard, P3 auth.uid() null check, D1 acquired_at/inbound_date coalesce) retained verbatim. Idempotent on replay. See Story 3.6 dev notes "Format-Korrektur 2026-05-04". |
| `00058+`       | Epic 3–9 stories. Range gets reserved when the story is created.  |
